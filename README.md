...
Connector live-stream watch rooms can also offer an optional generated `3D Stream` mode. Participants who choose it receive on-demand Half SBS or Full SBS HLS segments created from the 2D source and cached separately from normal 2D segments. The default near-real-time path is the `pipeline-depth-small-balanced` pipeline: Depth Anything V2 Small at `0.33x` internal depth inference, a sparse four-frame depth cadence with temporal interpolation, lightweight temporal smoothing, and OpenCV inpaint fill tuned for 720p on an M3 MacBook Pro. Use `pipeline-depth-small-fast` for a snappier `0.25x` inference fallback with an eight-frame depth cadence and remap fill. Tune the horizontal depth offset with `FILE_PIPE_HLS_STEREO3D_DEPTH_PERCENT` when needed.

Depth-aware stereo processors are selected from the Player controls or by setting `FILE_PIPE_HLS_STEREO3D_PROCESSOR` / `FILE_PIPE_HLS_STEREO3D_REALTIME_PROCESSOR`. Prepared 3D cache builds default to Depth Anything V2 Base at `0.6x` internal inference, no inference crop, temporal context across neighboring segments, 18-frame stabilization windows with six-frame overlap, two HLS segments per processing batch, and OpenCV inpaint fill for disocclusions. Depth Anything V3 Base is available for true DA3 sequence inference, but its default prepared-cache window is two frames with one-frame overlap to fit Apple Silicon MPS memory at high source resolutions. Tune the prepared-cache defaults with `FILE_PIPE_HLS_STEREO3D_PREBUILD_PROCESSOR`, `FILE_PIPE_HLS_STEREO3D_PREBUILD_INFERENCE_SCALE`, `FILE_PIPE_HLS_STEREO3D_PREBUILD_INFERENCE_CROP_PERCENT`, `FILE_PIPE_HLS_STEREO3D_PREBUILD_TEMPORAL_CONTEXT_SEGMENTS`, `FILE_PIPE_HLS_STEREO3D_PREBUILD_DA3_TEMPORAL_WINDOW_FRAMES`, `FILE_PIPE_HLS_STEREO3D_PREBUILD_DA3_TEMPORAL_OVERLAP_FRAMES`, `FILE_PIPE_HLS_STEREO3D_PREBUILD_DA2_TEMPORAL_WINDOW_FRAMES`, `FILE_PIPE_HLS_STEREO3D_PREBUILD_DA2_TEMPORAL_OVERLAP_FRAMES`, `FILE_PIPE_HLS_STEREO3D_PREBUILD_SEGMENT_BATCH_COUNT`, and `FILE_PIPE_HLS_STEREO3D_PREBUILD_TEMPORAL_SMOOTHING`. Supported IDs are `ffmpeg-shift`, real-time pipelines `pipeline-depth-small-balanced` and `pipeline-depth-small-fast`, `depth-anything-v2-small`, `depth-anything-v2-base`, `depth-anything-v3-small`, `depth-anything-v3-base`, `coreml-depth-anything-v2-small`, and experimental `webgpu-depth-anything-v2-small`. The WebGPU option is viewer-side and looks for a browser adapter at `window.FilePipeLocalDepth3dAdapter`.

To enable the local Depth Anything helpers used by `depth-anything-v2-small`, `depth-anything-v2-base`, `depth-anything-v3-small`, `depth-anything-v3-base`, and `coreml-depth-anything-v2-small`, run:

```bash
scripts/setup_depth_processors.sh
```

The setup script installs a dedicated helper under `~/Library/Application Support/File Pipe/depth-processors` on macOS, creates a separate Python virtualenv, installs the optional ML dependencies, and warms up the Depth Anything V2 Small, Depth Anything V2 Base, Depth Anything V3 Base, and Apple Core ML Depth Anything V2 Small models. The Local connector auto-discovers that helper, including when launched from `File Pipe Connector.app`. Manual per-segment command hooks are still supported with `FILE_PIPE_DEPTH_ANYTHING_V2_SMALL_COMMAND`, `FILE_PIPE_DEPTH_ANYTHING_V2_BASE_COMMAND`, `FILE_PIPE_DEPTH_ANYTHING_V3_SMALL_COMMAND`, `FILE_PIPE_DEPTH_ANYTHING_V3_BASE_COMMAND`, `FILE_PIPE_COREML_DEPTH_ANYTHING_V2_SMALL_COMMAND`, or the generic `FILE_PIPE_STEREO3D_COMMAND`. Command templates receive `{input}`, `{output}`, `{start}`, `{duration}`, `{layout}`, `{video_profile}`, `{processor}`, `{depth_percent}`, `{resolution_scale}`, `{inference_scale}`, `{inference_crop_percent}`, `{stereo_fill}`, `{inpaint_radius}`, `{encoder_preset}`, and `{depth_frame_stride}`.

Depth Anything prepared-cache builds use a resumable whole-cache helper instead of isolated segment jobs. Each HLS cache directory stores `.cache-metadata.json` for the source/settings and `.prebuild-manifest.json` for the depth run settings, completed segments, and resume status. If a temporal cache build is interrupted, the connector can restart the helper and only missing segment files are regenerated with the same temporal context and settings. Override the full-cache command with `FILE_PIPE_DEPTH_ANYTHING_PREBUILD_COMMAND`, `FILE_PIPE_DEPTH_ANYTHING_V2_PREBUILD_COMMAND`, or `FILE_PIPE_DEPTH_ANYTHING_V3_PREBUILD_COMMAND`; templates receive `{input}`, `{output_dir}`, `{metadata}`, `{layout}`, `{video_profile}`, `{processor}`, `{depth_percent}`, `{resolution_scale}`, `{inference_scale}`, `{inference_crop_percent}`, `{segment_seconds}`, `{temporal_context_segments}`, `{temporal_window_frames}`, `{temporal_overlap_frames}`, `{segment_batch_count}`, and `{temporal_smoothing}`.

The connector bounds expensive background work so segment prefetch and Stable MP4 jobs do not create unbounded threads. Tune `FILE_PIPE_TRANSCODE_WORKERS` for concurrent Stable MP4 jobs, `FILE_PIPE_HLS_PREFETCH_WORKERS` for concurrent segment prefetch jobs, and `FILE_PIPE_HLS_STEREO3D_PREFETCH_SEGMENTS` for the smaller 3D/depth prefetch window. Completed HLS segments and MP4 caches survive restarts. In-progress Stable MP4 jobs write a small `.job.json` manifest next to the target cache file and are restarted when the connector comes back up; incomplete non-temporal HLS segments are regenerated on demand.
